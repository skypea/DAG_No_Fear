...
def print_to_file( args,
    ...
    pre_use_l2_string = ['_zeroInit', '_notearInit', '_notearInitNT']

    method_info = args.methods + search_string[search_result] + '_'+ str(args.data_variable_size) + \
                  '_' + args.data_type + '_' + str(args.data_sample_size) + '_' +\
                  str(args.graph_type) + '_' + str(args.graph_sem_type) + \
                  '_' + str(args.graph_degree) +  pre_use_l2_string[args.pre_use_l2] +\
                  nonzero_string[args.zero_mean] + '_PrehTol_' + str(args.pre_h_tol) + '_hTol_' + str(args.h_tol)
    if not args.minimize_Z:
        method_info += '_noMinZ'
    if not args.revEdges:
        method_info += '_noRev'
    if args.methods in ['MMHC','MMPC']:
        method_info += '_alphaCIT_' + str(args.alphaCIT)

    save_path = os.path.join('./results', 'nips_2020')
    output_file_name = os.path.join(save_path, method_info)
    ...
    df = np.hstack((result_time, result_shd, result_nnz, result_h, result_extra, result_missing, result_reverse))
    df = pd.DataFrame(df, columns=['time', 'SHD', 'nnz', 'h', 'extra', 'missing', 'reverse'])
